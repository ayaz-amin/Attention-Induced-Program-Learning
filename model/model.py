...
class AttentionInduction(nn.Module):
    '''
    The full (uncoditional) Attention Induced Program Learning model. The model
...
class ConditionalAttentionInduction(nn.Module):
    '''
    The full (conditional) Attention Induced Program Learning model. The model
    consists of three sub-models:
        a) Type model
        b) Rendering model
        c) Deformation (token) model
    ...
    def __init__(self, num_classes, max_k=5, num_parts=10, filter_size=(64, 64), image_shape=(1, 105, 105)):
        super(ConditionalAttentionInduction, self).__init__()
        '''
        Attributes
        ----------
        ...
        height, width = image_shape[1], image_shape[2]
        self.type_model = ConditionalTypeModel(num_classes=num_classes, max_k=max_k, num_parts=num_parts, image_shape=(height, width))
        self.rendering_model = RenderingModel(num_parts=num_parts, filter_size=filter_size, image_shape=(height, width))
        self.token_model = DeformationModel(image_shape=image_shape)
